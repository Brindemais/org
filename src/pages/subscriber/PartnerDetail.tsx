...
export default function SubscriberPartnerDetail() {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    Promise.all([
      supabase.rpc('list_public_partners').select('id, trade_name, category, address, neighborhood, city, state, opening_hours, logo_url, whatsapp').eq('id', id).maybeSingle(),
      ...
      supabase.rpc('list_public_products').select('*').eq('partner_id', id).order('is_gift', { ascending: false }),
